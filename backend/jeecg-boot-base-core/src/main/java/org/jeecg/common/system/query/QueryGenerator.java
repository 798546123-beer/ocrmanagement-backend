...
public class QueryGenerator {
	...
	private static void installMplus(QueryWrapper<?> queryWrapper, Object searchObj, Map<String, String[]> parameterMap, Map<String, QueryRuleEnum> customRuleMap) {
		/*
		 * 注意:权限查询由前端配置数据规则 当一个人有多个所属公司时候 可以在规则配置包含条件 orgCode 包含 #{sys_org_code}
		但是不支持在自定义SQL中写orgCode in #{sys_org_code} 
		当一个人只有一个公司 就直接配置等于条件: orgCode 等于 #{sys_org_code} 或者配置自定义SQL: orgCode = '#{sys_org_code}'
		*/
	...
	public static Map<String, SysPermissionDataRuleModel> getRuleMap() {
		...
		//update-begin-author:taoyan date:2023-6-1 for:QQYUN-5441 【简流】获取多个用户/公司/角色 设置公司查询 报错
		try {
