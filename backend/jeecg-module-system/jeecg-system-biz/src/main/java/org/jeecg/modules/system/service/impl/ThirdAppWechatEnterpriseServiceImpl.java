...
public class ThirdAppWechatEnterpriseServiceImpl implements IThirdAppService {
    ...
    public SyncInfoVo syncLocalDepartmentToThirdApp(String ids) {
        ...
        if (departments == null) {
            syncInfo.addFailInfo("获取企业微信所有公司失败！");
            return syncInfo;
        }
        // 删除企业微信有但本地没有的公司（以本地公司数据为主）(以为企业微信不能创建同名公司，所以只能先删除）
        ...
        // -- 企业微信不能创建新的顶级公司，所以新的顶级公司的parentId就为1
    ...
    /**
     * 递归删除公司以及子公司，由于企业微信不允许删除带有成员和子公司的公司，所以需要递归删除下子公司，然后把公司成员移动端根公司下
     * @param children
     * @param accessToken
     * @param ifLocal
     */
...
//                this.syncUserCollectErrInfo(errCode, sysUser, errInfo);
    ...
    public SyncInfoVo syncThirdAppDepartmentToLocal(Integer tenantId, Map<String,String> map) {
        ...
        List<Department> departments = JwDepartmentAPI.getAllDepartment(accessToken);
        if (departments == null) {
            syncInfo.addFailInfo("企业微信公司信息获取失败！");
            return syncInfo;
        }
        ...
        if (departmentTreeList != null && departmentTreeList.size() != 0) {
            for (JwDepartmentTreeVo departmentTree : departmentTreeList) {
                ...
                if (sysDepart != null) {
                    ...
                    //update-begin---author:wangshuai---date:2024-04-10---for:【issues/6017】企业微信同步公司时没有最顶层的公司名，同步用户时，用户没有公司信息---
                    if (sysParentId != null && !"0".equals(sysParentId)) {
                    //update-end---author:wangshuai---date:2024-04-10---for:【issues/6017】企业微信同步公司时没有最顶层的公司名，同步用户时，用户没有公司信息---
...
//    }
    ...
    private boolean syncDepartCollectErrInfo(Exception e, Department department, SyncInfoVo syncInfo) {
        ...
        String str = String.format("公司 %s(%s) 同步失败！错误信息：%s", department.getName(), department.getId(), msg);
        syncInfo.addFailInfo(str);
        return false;
    }
    ...
    private User sysUserToQwUser(SysUser sysUser, User user) {
        ...
        if (departList != null) {
            ...
            // 个数必须和参数department的个数一致，表示在所在的公司内是否为上级。1表示为上级，0表示非上级。在审批等应用里可以用来标识上级审批人
    ...
    /**
     * 【同步公司】将SysDepartTreeModel转为企业微信的Department对象（创建新公司）
     */
    ...
    /**
     * 【同步公司】将SysDepartTreeModel转为企业微信的Department对象
     */
    ...
    /**
     * 【同步公司】将企业微信的Department对象转为SysDepart
     */
     ...
     * @param tenantId 租户id
     * @param idsMap 公司id集合 key为企业微信的id value 为系统公司的id
     * @param jwUserDepartJson
     */
