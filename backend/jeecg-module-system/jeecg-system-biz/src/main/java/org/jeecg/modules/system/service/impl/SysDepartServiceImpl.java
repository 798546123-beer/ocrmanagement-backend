...
public class SysDepartServiceImpl extends ServiceImpl<SysDepartMapper, SysDepart> implements ISysDepartService {
	...
	public List<SysDepartTreeModel> queryMyDeptTreeList(String departIds) {
		...
		//update-begin---author:wangshuai---date:2023-12-01---for:【QQYUN-7320】查询公司没数据，导致报错空指针---
		if(ArrayUtil.isEmpty(codeArr)){
			return null;
		}
		//update-end---author:wangshuai---date:2023-12-01---for:【QQYUN-7320】查询公司没数据，导致报错空指针---
	...
	/**
	 * queryTreeList 对应 queryTreeList 查询所有的公司数据,以树结构形式响应给前端
	 */
	@Override
	...
	public List<SysDepartTreeModel> queryTreeList() {
        ...
        //update-begin---author:wangshuai ---date:20220307  for：[JTC-119]在公司管理菜单下设置公司负责人 创建用户的时候不需要处理
		//设置用户id,让前台显示
	...
	/**
	 * saveDepartData 对应 add 保存用户在页面添加的新的公司对象数据
	 */
	@Override
	@Transactional(rollbackFor = Exception.class)
	public void saveDepartData(SysDepart sysDepart, String username) {
		if (sysDepart != null && username != null) {
			//update-begin---author:wangshuai ---date:20230216  for：[QQYUN-4163]给公司表加个是否有子节点------------
			...
			//update-end---author:wangshuai ---date:20230216  for：[QQYUN-4163]给公司表加个是否有子节点------------
			//String s = UUID.randomUUID().toString().replace("-", "");
			sysDepart.setId(IdWorker.getIdStr(sysDepart));
			// 先判断该对象有无父级ID,有则意味着不是最高级,否则意味着是最高级
			// 获取父级ID
			String parentId = sysDepart.getParentId();
			//update-begin--Author:baihailong  Date:20191209 for：公司编码规则生成器做成公用配置
			JSONObject formData = new JSONObject();
			formData.put("parentId",parentId);
			String[] codeArray = (String[]) FillRuleUtil.executeRule(FillRuleConstant.DEPART,formData);
			//update-end--Author:baihailong  Date:20191209 for：公司编码规则生成器做成公用配置
            ...
            //update-end---author:wangshuai ---date:20220307  for：[JTC-119]在公司管理菜单下设置公司负责人 创建用户的时候不需要处理
         }

	}
	
	/**
	 * saveDepartData 的调用方法,生成公司编码和公司类型（作废逻辑）
	 * @deprecated
	 * @param parentId
	 * @return
	 */
	...
	/**
	 * removeDepartDataById 对应 delete方法 根据ID删除相关公司数据
	 * 
	 */
	/*
	 * @Override
	...
	/**
	 * updateDepartDataById 对应 edit 根据公司主键来更新对应的公司数据
	 */
	@Override
	@Transactional(rollbackFor = Exception.class)
	...
	public void deleteBatchWithChildren(List<String> ids) {
		...
		for(String id: ids) {
			...
			//update-begin---author:wangshuai ---date:20230712  for：【QQYUN-5757】批量删除公司时未正确置为叶子节点 ------------
			...
			//update-end---author:wangshuai ---date:20230712  for：【QQYUN-5757】批量删除公司时未正确置为叶子节点 ------------
		}
	...
	public List<SysDepartTreeModel> searchByKeyWord(String keyWord,String myDeptSearch,String departIds) {
		...
		//myDeptSearch不为空时为我的公司搜索，只搜索所负责公司
		if(!StringUtil.isNullOrEmpty(myDeptSearch)){
			//departIds 为空普通用户或没有管理公司
			if(StringUtil.isNullOrEmpty(departIds)){
				return newList;
			}
			//根据公司id获取所负责公司
			String[] codeArr = this.getMyDeptParentOrgCode(departIds);
			//update-begin-author:taoyan date:20220104 for:/issues/3311 当用户属于两个公司的时候，且这两个公司没有上下级关系，我的公司-公司名称查询条件模糊搜索失效！
			if (codeArr != null && codeArr.length > 0) {
				query.nested(i -> {
			...
			//update-end-author:taoyan date:20220104 for:/issues/3311 当用户属于两个公司的时候，且这两个公司没有上下级关系，我的公司-公司名称查询条件模糊搜索失效！
			query.eq(SysDepart::getDelFlag, CommonConstant.DEL_FLAG_0.toString());
		}
	...
	public List<SysDepartTreeModel> queryTreeListByPid(String parentId,String ids, String primaryKey) {
        ...
        //update-begin---author:wangshuai ---date:20220316  for：[JTC-119]在公司管理菜单下设置公司负责人 创建用户的时候不需要处理
        //设置用户id,让前台显示
        this.setUserIdsByDepList(list);
        //update-end---author:wangshuai ---date:20220316  for：[JTC-119]在公司管理菜单下设置公司负责人 创建用户的时候不需要处理
    ...
    //update-begin---author:wangshuai ---date:20200308  for：[JTC-119]在公司管理菜单下设置公司负责人，新增方法添加公司负责人、删除负责公司负责人、查询公司对应的负责人
    /**
     * 通过用户id设置负责公司
     * @param sysDepart SysDepart公司对象
    ...
    private void updateChargeDepart(SysDepart sysDepart) {
        ...
        if(oConvertUtils.isEmpty(directorIds)){
            ...
            //如果用户id不为空但是用户原来负责公司的用户id为空
            this.addDepartByUserIds(sysDepart,directorIds);
        }else{
            //都不为空，需要比较，进行添加或删除
            //找到新的负责公司用户id与原来负责公司的用户id，进行删除
    ...
    private void deleteChargeDepId(String departId,String userId){
        //先查询负责公司的用户id,因为负责公司的id使用逗号拼接起来的
    ...
    /**
     * 通过公司集合为公司设置用户id，用于前台展示
     * @param departList 公司集合
     */
    ...
    //update-end---author:wangshuai ---date:20200308  for：[JTC-119]在公司管理菜单下设置公司负责人，新增方法添加公司负责人、删除负责公司负责人、查询公司对应的负责人
	 ...
	 * @param path 公司路径
	 * @param tenantId 租户id
	 * @param childrenDepart 子公司
	 * @param departIdList 公司id集合
	 */
	...
	/**
	 * 添加公司
	 * @param departNameUrl 公司路径
	 * @param departName 公司名称
	 * @param sysDepart 公司类
	 * @param errorMessageList 错误集合
	 * @param tenantId 租户id
	 * @param departMap 公司数组。避免存在公司信息再次查询 key 存放公司路径 value 存放公司对象
	...
	private Boolean addDepartByName(String departNameUrl,String departName,SysDepart sysDepart,List<String> errorMessageList,Integer tenantId,Map<String,SysDepart> departMap, int num) {
		...
		if(oConvertUtils.isEmpty(departNameUrl) && oConvertUtils.isEmpty(departName)){
			//公司路径为空
			errorMessageList.add("第 " + lineNumber + " 行：记录公司路径或者公司名称为空禁止导入");
			return false;
		}
		//获取公司名称路径
		String name = "";
		...
		if(!name.equals(departName)){
			//公司名称已存在
			errorMessageList.add("第 " + lineNumber + " 行：记录公司路径:”"+departNameUrl+"“"+"和公司名称：“"+departName+"“不一致，请检查！");
			return false;
		}else{
			String parentId = "";
			//判断是否包含“/”
			if(departNameUrl.contains(SymbolConstant.SINGLE_SLASH)){
				...
				if(departMap.containsKey(departNames)){
					...
					if(oConvertUtils.isNotEmpty(parentIdByName)){
						...
						//公司名称已存在
						errorMessageList.add("第 " + lineNumber + " 行：记录公司名称“"+departName+"”上级不存在，请检查！");
						return false;
					}
				}
			}
			...
			//update-begin---author:wangshuai ---date:20230721  for：一个租户公司名称可能有多个------------
			...
			//update-end---author:wangshuai ---date:20230721  for：一个租户公司名称可能有多个------------
			if(null != parentDept) {
				//公司名称已存在
				errorMessageList.add("第 " + lineNumber + " 行：记录公司名称“"+departName+"”已存在，请检查！");
				return false;
			}else{
	 ...
	 * @param departName 公司名称
	 * @param tenantId 租户id
	 * @param sysDepart 公司对象
	 * @param count 公司路径下标
	 * @param departNameUrls 公司路径
	 * @param departNum 公司路径的数量
	 * @param name 公司路径的数量
	 * @param departMap 存放公司的数据 key 存放公司路径 value 存放公司对象
	 */
	private String getDepartListByName(String departName, Integer tenantId, SysDepart sysDepart,String[] departNameUrls, int count, int departNum,String name,Map<String,SysDepart> departMap) {
		...
		if(null != parentDept){
			...
			//查询出来的公司名称和公司路径中的公司名称作比较，如果不存在直接返回空
			if(parentDept.getDepartName().equals(departNameUrls[count])){
				count = count + 1;
				//数量和公司数量相等说明已经到最后一位了，直接返回公司id
				if(count == departNum){
			...
			if(izExport){
				if(oConvertUtils.isNotEmpty(departExportVo.getOrgCode())){
					...
					if(null != depart){
						...
						//公司名称已存在
						errorMessageList.add("第 " + num + " 行：记录公司名称“"+departExportVo.getDepartName()+"”公司编码重复，请检查！");
						continue;
					}
					...
					if(departNameUrl.contains(SymbolConstant.SINGLE_SLASH)){
						...
						if(!sysDepart.getOrgCode().equals(departExportVo.getOrgCode())){
							...
							//公司名称已存在
							errorMessageList.add("第 " + num + " 行：记录公司名称“"+departExportVo.getDepartName()+"”公司编码规则不匹配，请检查！");
							continue;
						}
					}
