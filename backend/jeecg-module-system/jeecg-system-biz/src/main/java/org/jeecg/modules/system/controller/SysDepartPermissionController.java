...
public class SysDepartPermissionController extends JeecgController<SysDepartPermission, ISysDepartPermissionService> {
	 ...
	 * @return
	 */
	@ApiOperation(value="公司权限表-分页列表查询", notes="公司权限表-分页列表查询")
	 ...
	 * @return
	 */
	@ApiOperation(value="公司权限表-添加", notes="公司权限表-添加")
	 ...
	 * @return
	 */
	@ApiOperation(value="公司权限表-编辑", notes="公司权限表-编辑")
	 ...
	 * @return
	 */
	@ApiOperation(value="公司权限表-通过id删除", notes="公司权限表-通过id删除")
	 ...
	 * @return
	 */
	@ApiOperation(value="公司权限表-批量删除", notes="公司权限表-批量删除")
	 ...
	 * @return
	 */
	@ApiOperation(value="公司权限表-通过id查询", notes="公司权限表-通过id查询")
	...
	*/
	...
	public ModelAndView exportXls(HttpServletRequest request, SysDepartPermission sysDepartPermission) {
	  return super.exportXls(request, sysDepartPermission, SysDepartPermission.class, "公司权限表");
	}
	...
	*/
	...
	public Result<?> saveDatarule(@RequestBody JSONObject jsonObject) {
		try {
			...
			log.info("保存数据规则>>"+"菜单ID:"+permissionId+"公司ID:"+ departId+"数据权限ID:"+dataRuleIds);
			LambdaQueryWrapper<SysDepartPermission> query = new LambdaQueryWrapper<SysDepartPermission>()
				 .eq(SysDepartPermission::getPermissionId, permissionId)
				 .eq(SysDepartPermission::getDepartId,departId);
			SysDepartPermission sysDepartPermission = sysDepartPermissionService.getOne(query);
			if(sysDepartPermission==null) {
				return Result.error("请先保存公司菜单权限!");
			}else {
	 ...
	 public Result<String> saveDeptRolePermission(@RequestBody JSONObject json) {
		 ...
		 try {
			 ...
			 result.success("保存成功！");
             //update-begin---author:wangshuai ---date:20220316  for：[VUEN-234]公司角色授权添加敏感日志------------
             LoginUser loginUser = (LoginUser) SecurityUtils.getSubject().getPrincipal();
             baseCommonService.addLog("修改公司角色ID:"+roleId+"的权限配置，操作人： " +loginUser.getUsername() ,CommonConstant.LOG_TYPE_2, 2);
             //update-end---author:wangshuai ---date:20220316  for：[VUEN-234]公司角色授权添加敏感日志------------
             log.info("======公司角色授权成功=====耗时:" + (System.currentTimeMillis() - start) + "毫秒");
		 } catch (Exception e) {
