...
public class ThirdAppDingtalkServiceImpl implements IThirdAppService {
    ...
    // update：2022-1-21，updateBy：sunjianlei; for 【JTC-704】【钉钉】公司同步成功，实际没成，后台提示ip白名单
    @Override
    public SyncInfoVo syncLocalDepartmentToThirdApp(String ids) {
        ...
        // 删除钉钉有但本地没有的公司（以本地公司数据为主）（钉钉不能创建同名公司，只能先删除）
        ...
        for (Response<Department> departmentRes : departments) {
            ...
            // 钉钉不允许删除带有用户的公司，所以需要判断下，将有用户的公司的用户移动至根公司
        ...
        // -- 钉钉不能创建新的顶级公司，所以新的顶级公司的parentId就为1
...
//    }
    ...
    public void syncDepartmentToLocalRecursion(List<JdtDepartmentTreeVo> departmentTreeList, String sysParentId, String username, SyncInfoVo syncInfo, String accessToken,Boolean syncUser,Integer tenantId) {
        ...
        if (departmentTreeList != null && departmentTreeList.size() != 0) {
            ...
            for (JdtDepartmentTreeVo departmentTree : departmentTreeList) {
                ...
                //update-begin---author:wangshuai---date:2024-04-10---for:【issues/6017】钉钉同步公司时没有最顶层的公司名，同步用户时，用户没有公司信息---
                ...
                //update-end---author:wangshuai---date:2024-04-10---for:【issues/6017】钉钉同步公司时没有最顶层的公司名，同步用户时，用户没有公司信息---
                ...
                if (sysDepart != null) {
                    ...
                    try {
                        ...
                        String str = String.format("公司 %s 更新成功！", updateSysDepart.getDepartName());
                    ...
                    try {
                        ...
                        String str = String.format("公司 %s 创建成功！", newSysDepart.getDepartName());
                        syncInfo.addSuccessInfo(str);
                        //判断是否需要同步用户
                        if(syncUser){
    ...
    private boolean syncDepartCollectErrInfo(Exception e, Department department, SyncInfoVo syncInfo) {
        ...
        String str = String.format("公司 %s(%s) 同步失败！错误信息：%s", department.getName(), department.getDept_id(), msg);
    ...
    private boolean syncDepartCollectErrInfo(Response<?> response, SysDepartTreeModel depart, SyncInfoVo syncInfo) {
        if (!response.isSuccess()) {
            String str = String.format("公司 %s(%s) 同步失败！错误码：%s——%s", depart.getDepartName(), depart.getOrgCode(), response.getErrcode(), response.getErrmsg());
            syncInfo.addFailInfo(str);
            return false;
        } else {
            String str = String.format("公司户 %s(%s) 同步成功！", depart.getDepartName(), depart.getOrgCode());
            syncInfo.addSuccessInfo(str);
            return true;
        }
    }
...
//    }
    ...
    /**
     * 【同步公司】将SysDepartTreeModel转为【钉钉】的Department对象（创建新公司）
     */
    ...
    /**
     * 【同步公司】将SysDepartTreeModel转为【钉钉】的Department对象
     */
    ...
    /**
     * 【同步公司】将【钉钉】的Department对象转为SysDepartTreeModel
     */
