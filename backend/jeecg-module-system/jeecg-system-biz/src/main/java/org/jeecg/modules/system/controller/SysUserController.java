...
public class SysUserController {
    ...
    /**
     * 生成在添加用户情况下没有主键的问题,返回给前端,根据该id绑定公司数据
     *
     * @return
     */
    ...
    /**
     * 根据 orgCode 查询用户，包括子公司下的用户
     * 若某个用户包含多个公司，则会显示多条记录，可自行处理成单条记录
     */
    ...
    /**
     * 根据 orgCode 查询用户，包括子公司下的用户
     * 针对通讯录模块做的接口，将多个公司的用户合并成一条记录，并转成对前端友好的格式
     */
    ...
    public Result<?> queryByOrgCodeForAddressList(
        ...
        for (SysUserSysDepartModel item : list) {
            ...
            if (getModel != null) {
                ...
                // 将用户对象转换为json格式，并将公司信息合并到 json 中
    ...
    public Result<SysUserDepart> deleteUserInDepart(@RequestParam(name="depId") String depId,
        ...
        try {
            ...
            if(b){
                ...
                result.error500("当前选中公司与用户无关联关系!");
            }
        }catch(Exception e) {
            log.error(e.getMessage(), e);
            result.error500("删除失败！");
...
//	}
	...
	public Result<?> getUserSectionInfoByToken(HttpServletRequest request, @RequestParam(name = "token", required = false) String token) {
		try {
			...
			map.put("sysOrgCode", sysUser.getOrgCode()); // 当前登录用户公司编号
    ...
    public Result getRecycleBin() {
        ...
        if (logicDeletedUserList.size() > 0) {
            ...
            // step.2 通过 userIds，一次性查询用户的所属公司名字
            Map<String, String> useDepNames = sysUserService.getDepNamesByUserIds(userIds);
