	...
	public Result<List<SysDepartTreeModel>> queryMyDeptTreeList() {
		...
		try {
			if(oConvertUtils.isNotEmpty(user.getUserIdentity()) && user.getUserIdentity().equals( CommonConstant.USER_IDENTITY_2 )){
				//update-begin--Author:liusq  Date:20210624  for:公司查询ids为空后的前端显示问题 issues/I3UD06
				String departIds = user.getDepartIds();
				if(StringUtils.isNotBlank(departIds)){
					List<SysDepartTreeModel> list = sysDepartService.queryMyDeptTreeList(departIds);
					result.setResult(list);
				}
				//update-end--Author:liusq  Date:20210624  for:公司查询ids为空后的前端显示问题 issues/I3UD06
...
//			}
   ...
   public Result<SysDepart> delete(@RequestParam(name="id",required=true) String id) {
	...
	/**
	 * 批量删除 根据前端请求的多个ID,对数据库执行删除相关公司数据的操作
	 * 
	 * @param ids
	 * @return
	 */
	...
	/**
	 * 查询数据 添加或编辑页面对该方法发起请求,以树结构形式加载所有公司的名称,方便用户的操作
	 * 
	 * @return
	 */
...
//		}
	...
	public Result<List<SysDepartTreeModel>> searchBy(@RequestParam(name = "keyWord", required = true) String keyWord,@RequestParam(name = "myDeptSearch", required = false) String myDeptSearch) {
		...
		//公司查询，myDeptSearch为1时为我的公司查询，登录用户为上级时查只查负责公司下数据
		LoginUser user = (LoginUser) SecurityUtils.getSubject().getPrincipal();
    ...
    public ModelAndView exportXls(SysDepart sysDepart,HttpServletRequest request) {
		...
		//update-begin---author:wangshuai---date:2023-10-19---for:【QQYUN-5482】系统的公司导入导出也可以改成敲敲云模式的公司路径---
		//// Step.1 组装查询条件
        ...
        //导出文件名称
        mv.addObject(NormalExcelConstants.FILE_NAME, "公司列表");
        ...
        mv.addObject(NormalExcelConstants.PARAMS, new ExportParams("导入规则：\n" +
				"1、标题为第三行，公司路径和公司名称的标题不允许修改，否则会匹配失败；第四行为数据填写范围;\n" +
				"2、公司路径用英文字符/分割，公司名称为公司路径的最后一位;\n" +
				"3、公司从一级名称开始创建，如果有同级就需要多添加一行，如研发部/研发一部;研发部/研发二部;\n" +
				"4、自定义的公司编码需要满足规则才能导入。如一级公司编码为A01,那么子公司为A01A01,同级子公司为A01A02,编码固定为三位，首字母为A-Z,后两位为数字0-99，依次递增;", "导出人:"+user.getRealname(), "导出信息"));
        mv.addObject(NormalExcelConstants.DATA_LIST, sysDepartExportVos);
		//update-end---author:wangshuai---date:2023-10-19---for:【QQYUN-5482】系统的公司导入导出也可以改成敲敲云模式的公司路径---
    ...
    /**
     * 通过excel导入数据
	 * 公司导入方案1: 通过机构编码来计算出公司的父级ID,维护上下级关系;
	 * 公司导入方案2: 你也可以改造下程序,机构编码直接导入,先不设置父ID;全部导入后,写一个sql,补下父ID;
     *
     * @param request
     * @param response
     * @return
    ...
    public Result<?> importExcel(HttpServletRequest request, HttpServletResponse response) {
        ...
        for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
            ...
            try {
				//update-begin---author:wangshuai---date:2023-10-20---for: 注释掉原来的导入公司的逻辑---
//            	// orgCode编码长度
...
//                		sysDepart.setParentId("");
//					}
//                    //update-begin---author:liusq   Date:20210223  for：批量导入公司以后，不能追加下一级公司 #2245------------
//					sysDepart.setOrgType(sysDepart.getOrgCode().length()/codeLength+"");
//                    //update-end---author:liusq   Date:20210223  for：批量导入公司以后，不能追加下一级公司 #2245------------
//					sysDepart.setDelFlag(CommonConstant.DEL_FLAG_0.toString());
//                    //update-begin---author:wangshuai ---date:20220105  for：[JTC-363]公司导入 机构类别没有时导入失败，赋默认值------------
//					if(oConvertUtils.isEmpty(sysDepart.getOrgCategory())){
//					    sysDepart.setOrgCategory("1");
//                    }
//                    //update-end---author:wangshuai ---date:20220105  for：[JTC-363]公司导入 机构类别没有时导入失败，赋默认值------------
//					ImportExcelUtil.importDateSaveOne(sysDepart, ISysDepartService.class, errorMessageList, num, CommonConstant.SQL_INDEX_UNIQ_DEPART_ORG_CODE);
//					num++;
//                }
				//update-end---author:wangshuai---date:2023-10-20---for: 注释掉原来的导入公司的逻辑---
	...
	/**
	 * 通过公司id和租户id获取用户 【低代码应用: 用于选择公司负责人】
	 * @param departId
	 * @return
	 */
	...
	public ModelAndView appExportXls(SysDepart sysDepart,HttpServletRequest request) {
		...
		LoginUser user = (LoginUser) SecurityUtils.getSubject().getPrincipal();
		mv.addObject(NormalExcelConstants.PARAMS, new ExportParams("公司列表数据", "导出人:"+user.getRealname(), "导出信息"));
		mv.addObject(NormalExcelConstants.DATA_LIST, pageList);
		return mv;
	}

	/**
	 * 导入excel【低代码应用: 用于导出公司】
	 *
	 * @param request
	 */
