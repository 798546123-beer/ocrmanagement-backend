...
public class SysUserServiceImpl extends ServiceImpl<SysUserMapper, SysUser> implements ISysUserService {
	...
	public Result<IPage<SysUser>> queryPageList(HttpServletRequest req, QueryWrapper<SysUser> queryWrapper, Integer pageSize, Integer pageNo) {
		...
		//update-begin-Author:wangshuai--Date:20211119--for:【vue3】通过公司id查询用户，通过code查询id
		//公司ID
		...
		if (oConvertUtils.isNotEmpty(departId)) {
			...
			//update-begin---author:wangshuai ---date:20220322  for：[issues/I4XTYB]查询用户时，当公司id 下没有分配用户时接口报错------------
			if (oConvertUtils.listIsNotEmpty(userIds)) {
				queryWrapper.in("id", userIds);
			} else {
				return Result.OK();
			}
			//update-end---author:wangshuai ---date:20220322  for：[issues/I4XTYB]查询用户时，当公司id 下没有分配用户时接口报错------------
		}
		//用户ID
		...
		//update-end-Author:wangshuai--Date:20211119--for:【vue3】通过公司id查询用户，通过code查询id
...
//		}
		...
		//step.2 通过 useids，一次性查询用户的所属公司名字
		...
		if (userIds != null && userIds.size() > 0) {
			...
			pageList.getRecords().forEach(item -> {
				...
				//update-begin---author:wangshuai---date:2023-10-08---for:【QQYUN-6668】钉钉公司和用户同步，我怎么知道哪些用户是双向绑定成功的---
				...
				//update-end---author:wangshuai---date:2023-10-08---for:【QQYUN-6668】钉钉公司和用户同步，我怎么知道哪些用户是双向绑定成功的---
			});
		}
...
//		}
	...
	public void batchEditUsers(JSONObject json) {
		...
		if(oConvertUtils.isNotEmpty(selecteddeparts)) {
			...
			if(departList==null || departList.size()==0){
				log.error("batchEditUsers 根据租户ID没有找到公司>"+currentTenantId);
				return;
			}
	...
	public void doUpdateDepartInfo(UpdateDepartInfo info) {
		...
		if(depart!=null){
			...
			//update-begin---author:wangshuai ---date:20230303  for：公司负责人不能被删除------------
				...
				if(CollectionUtil.isNotEmpty(departChargeUsers)){
					//前端传过来用户列表id为空，说明数据库的负责公司人员均需要删除
					this.removeDepartmentManager(departChargeUserIdList,departChargeUsers,departId);
				}
			//update-end---author:wangshuai ---date:20230303  for：公司负责人不能被删除------------
	...
	public void editTenantUser(SysUser sysUser, String tenantId, String departs, String roles) {
		...
		this.updateById(user);
		//update-begin---author:wangshuai ---date:20230424  for：【QQYUN-5251】人员与公司：公司删除不掉------------
		...
		//update-end---author:wangshuai ---date:20230424  for：【QQYUN-5251】人员与公司：公司删除不掉------------
		//修改用户下的职位
	...
	public void updateTenantDepart(SysUser user, String tenantId, String departs) {
		...
		System.out.println("查询用户公司用时：" + (endTime - startTime) + "ms");
		//查询当前租户下公司和用户已关联的公司
		...
		System.out.println("修改公司角色用时：" + (endTime1 - startTime) + "ms");
		...
		System.out.println("修改用户公司用时：" + (endTime2 - startTime) + "ms");
	}
	...
	public ModelAndView exportAppUser(HttpServletRequest request) {
		...
		ExportParams exportParams = new ExportParams("导入规则：\n" +
				...
				"3、上下级公司用英文字符 / 连接，如 财务部/财务一部，多个公司或者职位用英文字符 ; 进行连接,如 财务部;研发部", "导出人:" + user.getRealname(), "导出信息");
	...
	public Result<?> importAppUser(HttpServletRequest request) {
		...
		for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
			...
			//存放公司的map;key为名称 value为SysDepart对象。避免多次导入和查询
	 ...
	 * @param depart    公司名称
	 * @param tenantId  租户id
	 * @param departMap 存放公司的map;key为名称 value为SysDepart对象。
	 */
