...
public class OpenCVQrCodePosition {
    ...
    static {
        try {
            // 指定 .dll 文件的绝对路径
            System.load("E:\\java_project\\ocrmanagement-backend\\backend\\test\\lib\\opencv_java455.dll");
            // 后续可以编写使用 OpenCV 的代码
        } catch (UnsatisfiedLinkError e) {
            System.err.println("无法加载本地库文件: " + e.getMessage());
        }
    }

    // 获取二维码图片位置信息
    public java.awt.Point[] getPosition(String imgUrl){
        ...
        java.awt.Point[] points =  this.centerCals(markContours);
        if(points != null){
            this.cutRect(src, points, 10);
        }
        return points;
    }

    //  裁剪指定区域
    public BufferedImage cutRect(Mat src, java.awt.Point[] points, int offset){
    ...
    private boolean capture(List<MatOfPoint> contours){
        java.awt.Point[] pointthree = this.centerCals(contours);
    ...
    //  计算夹角
    private double angle(java.awt.Point p1, java.awt.Point p2, java.awt.Point p3){
    ...
    // 转换为点
    private java.awt.Point[] centerCals(List<MatOfPoint> matOfPoint){
        if(matOfPoint == null || matOfPoint.size() == 0){
            return null;
        }
        java.awt.Point[] pointthree = new java.awt.Point[matOfPoint.size()];
    ...
    // 转换为中心点
    private java.awt.Point centerCal(MatOfPoint matOfPoint) {
        ...
        for (org.opencv.core.Point vertex : vertices) {
            if (vertex == null) {
        ...
        // 将 org.opencv.core.Point 转换为 java.awt.Point
        return new java.awt.Point((int) centerx, (int) centery);
    }
    ...
    public static void main(String[] args) {
        String img = "E:\\java_project\\ocrmanagement-backend\\backend\\test\\test.png";
        java.awt.Point[] points =  new OpenCVQrCodePosition().getPosition(img);
        if(points == null) {
        ...
        String qrCodeText = decodeQRcode("cutRect.jpg");
